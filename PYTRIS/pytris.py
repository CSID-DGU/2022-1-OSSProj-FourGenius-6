...
def set_initial_values():
    ...
    textsize = False
    # 튜토리얼에서 발생한 이벤트 상태 저장용 변수 (hard_10s, hard_3line, hard_full, multi_1P_break, multi_2P_break, multi_5break, multi_full, no_event)
    tutorial_event = ''
    # 튜토리얼에서 반복해서 발생하는 이벤트 발생 첫 회에서만 설명 띄울 수 있게, 발생 여부를 저장하는 딕셔너리
    tutorial_event_happened = {'hard_3line': False,
                ...
                if game_status != 'pvp':
                    ...
                    if restart_button.isOver_2(pos):
                        ...
                        if game_status == 'hard':
                            ...
                            hard = True
                            pygame.mixer.music.play(-1)
                        # if game_status == 'hard_tutorial':
                        #     initialize = True
                        #     time_attack = True
                        #     pygame.mixer.music.play(-1)
                        # if game_status == 'multi_tutorial':
                        #     initialize = True
                        #     multi_tutorial = True
                        #     pygame.mixer.music.play(-1)
                        ui_variables.click_sound.play()
...
=======
        ...
        help_surface = screen.convert_alpha()  # 투명 가능하도록
        help_surface.fill((0, 0, 0, 0))  # 투명한 검정색으로 덮기
        pygame.draw.rect(help_surface, (ui_variables.black_pause), [0, 0, int(
            board_width), int(board_height)])  # (screen, 색깔, 위치 x, y좌표, 너비, 높이)
